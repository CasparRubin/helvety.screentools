...
- **Live Draw**: global hotkey → fullscreen **Win32 layered host** hosting a **DesktopWindowXamlSource** island (vector markup only; no desktop BitBlt). **Esc** ends the session. **Default tools** (Rectangle modifier **Shift**): **Shift**+drag = animated rectangles; **Alt**+drag (without Ctrl) = arrows; **Ctrl**+drag = **straight segments** with uniform stroke (not arrow-shaft taper; no arrowhead); drag without a shape-tool modifier combination = freehand. **Other Rectangle modifier** options (Settings) change which key selects rectangles and how arrows are chosen; if the modifier is **Ctrl**, **Ctrl**+drag is rectangles only (straight line via Ctrl is disabled). **Rectangles, arrows, straight lines, and freehand strokes** use the same snap-border chrome as capture selection (gradients, dashing, pulse). **Does not require a save folder**; saving captures from capture mode still requires a writable save location.
