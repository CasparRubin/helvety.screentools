...
- **Settings layout**: the main **NavigationView** has a single left rail. **Settings** is an expandable group (PowerToys-style) with **General**, **Animations**, **Screen capture**, **Live Draw**, and **Danger zone**. **General** covers close-to-tray and editor performance. **Animations** sets global **snap-border intensity** (Subtle / Balanced / Bold) for both frozen-screen capture and Live Draw. **Screen capture** includes save folder, master **On/Off**, hotkey editor, capture quality, and overlay instructions. **Live Draw** includes a master **On/Off** toggle (shortcuts stay stored when off), global hotkey editor, and **Shape shortcuts** (per-tool modifiers plus fixed free draw and sparkle).
...
- **Navigation**: left **NavigationView** with **Screen Tools**, an expandable **Settings** group (**General**, **Animations**, **Screen capture**, **Live Draw**, **Danger zone**), and **About**. **About** shows a short product summary, a **compile-time build stamp** (`yyMMdd_HHmmss` from local clock when the project is built; see `GenerateAppBuildStamp` in `helvety.screentools.csproj`), and links to [helvety.com](https://helvety.com/) and the [GitHub repository](https://github.com/CasparRubin/helvety.screentools).
...
- **Snap-border overlay** (frozen-screen capture + Live Draw): shared animated chrome (gradients, dashing, pulse). Global **intensity** (Subtle / Balanced / Bold) is under **Settings → Animations**. The **image editor** uses separate stroke and effect controls for layers.
...
3. Use the navigation pane to switch between **Screen Tools**, **Settings** (expand for section items), and **About**.
4. Under **Settings**, choose a section:
   - **General**: **Notification area** (close-to-tray) and **Editor performance mode**.
   - **Animations**: **Snap border** intensity (global for capture selection and Live Draw).
   - **Screen capture**: **Save location** (required for frozen-screen capture and for **applying** a new capture shortcut), master toggle, **Listen**-based hotkey editor, **Capture quality**, and **Overlay** instructions.
   - **Live Draw**: master toggle, **Listen**-based hotkey editor, and **Shape shortcuts** (per-tool modifiers). **Live Draw** can be configured without a save folder.
   - **Danger zone**: reset all saved settings to defaults (captures on disk are not deleted).
